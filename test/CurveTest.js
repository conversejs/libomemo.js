/* vim: ts=4:sw=4 */
/* global assertEqualArrayBuffers */
...
describe("Curve", function () {
    ...
    describe("generateKeyPair", function () {
        it("returns a 32-byte private key and a 33-byte public key with 0x05 prefix", async function () {
            const keyPair = await libomemo.Curve.async.generateKeyPair();
        ...
        it("generates different keys each time", async function () {
            const kp1 = await libomemo.Curve.async.generateKeyPair();
            const kp2 = await libomemo.Curve.async.generateKeyPair();
    ...
    describe("createKeyPair", function () {
        it("derives the public key from a given private key", async function () {
            const kp = await libomemo.Curve.async.generateKeyPair();
            const kp2 = await libomemo.Curve.async.createKeyPair(kp.privKey);
        ...
        it("throws for invalid private key (not ArrayBuffer)", function () {
            assert.throw(
                () => libomemo.Curve.async.createKeyPair("notabuffer"),
                Error,
        ...
        it("throws for private key with wrong length", function () {
            ...
            assert.throw(
                () => libomemo.Curve.async.createKeyPair(shortKey),
                Error,
                "Invalid private key"
            );
        });
    ...
    describe("calculateAgreement", function () {
        it("produces identical shared secrets for both parties", async function () {
            const alice = await libomemo.Curve.async.generateKeyPair();
            const bob = await libomemo.Curve.async.generateKeyPair();

            const aliceShared = libomemo.Curve.async.calculateAgreement(bob.pubKey, alice.privKey);
            const bobShared = libomemo.Curve.async.calculateAgreement(alice.pubKey, bob.privKey);
        ...
        it("produces a 32-byte shared secret", async function () {
            ...
            const shared = await libomemo.Curve.async.calculateAgreement(bob.pubKey, alice.privKey);
        ...
        it("throws for invalid public key format (wrong length)", async function () {
            ...
            assert.throw(
                () => libomemo.Curve.async.calculateAgreement(badPub, kp.privKey),
                Error,
                "Invalid public key"
            );
        });
    ...
    describe("calculateSignature", function () {
        it("produces a 64-byte signature", async function () {
            ...
            const sig = await libomemo.Curve.async.calculateSignature(kp.privKey, message);
        ...
        it("throws for undefined message", async function () {
            const kp = await libomemo.Curve.async.generateKeyPair();
            assert.throw(
                () => libomemo.Curve.async.calculateSignature(kp.privKey, undefined),
                Error,
                "Invalid message"
            );
        });
    ...
    describe("verifySignature", function () {
        it("verifies a valid signature (resolves with undefined)", async function () {
            ...
            const sig = await libomemo.Curve.async.calculateSignature(kp.privKey, message);
            const result = await libomemo.Curve.async.verifySignature(kp.pubKey, message, sig);
        ...
        it("throws for invalid public key format", function () {
            ...
            assert.throw(
                () => libomemo.Curve.async.verifySignature(badPub, msg, sig),
                Error,
                "Invalid public key"
            );
        });
        ...
        it("throws for invalid signature length", async function () {
            ...
            assert.throw(
                () => libomemo.Curve.async.verifySignature(kp.pubKey, msg, badSig),
                Error,
                "Invalid signature"
            );
        });
