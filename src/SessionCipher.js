...
class SessionCipher {
            ...
            return Promise.all([
                ...
                .then((keys) => {
                    ...
                    return Internal.crypto
                        .encrypt(keys[0], buffer, keys[2].slice(0, 16))
                        .then((ciphertext) => {
    ...
    calculateRatchet(session, remoteKey, sending) {
        ...
        return Internal.crypto
            .ECDHE(remoteKey, util.toArrayBuffer(ratchet.ephemeralKeyPair.privKey))
            .then((sharedSecret) => {
                return Internal.HKDF(
                    sharedSecret,
                    ...
                    session[util.toString(ephemeralPublicKey)] = {
                        ...
                        chainType: sending
                            ? Internal.ChainType.SENDING
                            : Internal.ChainType.RECEIVING,
                    };
...
libsignal.SessionCipher = function (storage, remoteAddress) {
    ...
    this.decryptPreKeyWhisperMessage = (buffer, encoding) =>
        cipher.decryptPreKeyWhisperMessage(buffer, encoding);
    this.decryptWhisperMessage = (buffer, encoding) =>
        cipher.decryptWhisperMessage(buffer, encoding);
